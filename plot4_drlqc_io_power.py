...
def summarize(out_lq_list, out_wdrc_list, out_drce_list, out_drlqc_list, dist, noise_dist, path, num,  plot_results=True):
    ...
    if plot_results:
        ...
        for i in range(nx):
            ...
            if x_lqr_list != []:
                plt.plot(t, x_lqr_mean[:-1, i, 0], 'tab:red', label='LQG')
                plt.fill_between(t, x_lqr_mean[:-1, i, 0] + 0.3 * x_lqr_std[:-1, i, 0],
                                x_lqr_mean[:-1, i, 0] - 0.3 * x_lqr_std[:-1, i, 0], facecolor='tab:red', alpha=0.3)

            if x_wdrc_list != []:
                plt.plot(t, x_wdrc_mean[:-1, i, 0], 'tab:blue', label='WDRC')
                plt.fill_between(t, x_wdrc_mean[:-1, i, 0] + 0.3 * x_wdrc_std[:-1, i, 0],
                                x_wdrc_mean[:-1, i, 0] - 0.3 * x_wdrc_std[:-1, i, 0], facecolor='tab:blue', alpha=0.3)

            if x_drlqc_list != []:
                plt.plot(t, x_drlqc_mean[:-1, i, 0], 'tab:purple', label='DRLQC')
                plt.fill_between(t, x_drlqc_mean[:-1, i, 0] + 0.3 * x_drlqc_std[:-1, i, 0],
                                x_drlqc_mean[:-1, i, 0] - 0.3 * x_drlqc_std[:-1, i, 0], facecolor='tab:purple', alpha=0.3)

            if x_drce_list != []:
                plt.plot(t, x_drce_mean[:-1, i, 0], 'tab:green', label='WDR-CE')
                plt.fill_between(t, x_drce_mean[:-1, i, 0] + 0.3 * x_drce_std[:-1, i, 0],
                                x_drce_mean[:-1, i, 0] - 0.3 * x_drce_std[:-1, i, 0], facecolor='tab:green', alpha=0.3)
        ...
        for i in range(nu):
            ...
            if u_lqr_list != []:
                plt.plot(t, u_lqr_mean[:-1, i, 0], 'tab:red', label='LQG')
                plt.fill_between(t, u_lqr_mean[:-1, i, 0] + 0.25 * u_lqr_std[:-1, i, 0],
                                u_lqr_mean[:-1, i, 0] - 0.25 * u_lqr_std[:-1, i, 0], facecolor='tab:red', alpha=0.3)

            if u_wdrc_list != []:
                plt.plot(t, u_wdrc_mean[:-1, i, 0], 'tab:blue', label='WDRC')
                plt.fill_between(t, u_wdrc_mean[:-1, i, 0] + 0.25 * u_wdrc_std[:-1, i, 0],
                                u_wdrc_mean[:-1, i, 0] - 0.25 * u_wdrc_std[:-1, i, 0], facecolor='tab:blue', alpha=0.3)

            if u_drlqc_list != []:
                plt.plot(t, u_drlqc_mean[:-1, i, 0], 'tab:purple', label='DRLQC')
                plt.fill_between(t, u_drlqc_mean[:-1, i, 0] + 0.25 * u_drlqc_std[:-1, i, 0],
                                u_drlqc_mean[:-1, i, 0] - 0.25 * u_drlqc_std[:-1, i, 0], facecolor='tab:purple', alpha=0.3)

            if u_drce_list != []:
                plt.plot(t, u_drce_mean[:-1, i, 0], 'tab:green', label='WDR-CE')
                plt.fill_between(t, u_drce_mean[:-1, i, 0] + 0.25 * u_drce_std[:-1, i, 0],
                                u_drce_mean[:-1, i, 0] - 0.25 * u_drce_std[:-1, i, 0], facecolor='tab:green', alpha=0.3)
        ...
        for i in range(ny):
            if y_lqr_list != []:
                plt.plot(t, y_lqr_mean[:-1, i, 0], 'tab:red', label='LQG')
                plt.fill_between(t, y_lqr_mean[:-1, i, 0] + 0.25 * y_lqr_std[:-1, i, 0],
                                y_lqr_mean[:-1, i, 0] - 0.25 * y_lqr_std[:-1, i, 0], facecolor='tab:red', alpha=0.3)

            if y_wdrc_list != []:
                plt.plot(t, y_wdrc_mean[:-1, i, 0], 'tab:blue', label='WDRC')
                plt.fill_between(t, y_wdrc_mean[:-1, i, 0] + 0.25 * y_wdrc_std[:-1, i, 0],
                                y_wdrc_mean[:-1, i, 0] - 0.25 * y_wdrc_std[:-1, i, 0], facecolor='tab:blue', alpha=0.3)

            if y_drlqc_list != []:
                plt.plot(t, y_drlqc_mean[:-1, i, 0], 'tab:purple', label='DRLQC')
                plt.fill_between(t, y_drlqc_mean[:-1, i, 0] + 0.25 * y_drlqc_std[:-1, i, 0],
                                y_drlqc_mean[:-1, i, 0] - 0.25 * y_drlqc_std[:-1, i, 0], facecolor='tab:purple', alpha=0.3)

            if y_drce_list != []:
                plt.plot(t, y_drce_mean[:-1, i, 0], 'tab:green', label='WDR-CE')
                plt.fill_between(t, y_drce_mean[:-1, i, 0] + 0.25 * y_drce_std[:-1, i, 0],
                                y_drce_mean[:-1, i, 0] - 0.25 * y_drce_std[:-1, i, 0], facecolor='tab:green', alpha=0.3)
...
if __name__ == "__main__":
    ...
    if args.use_lambda:
        path = "./results/{}_{}/finite/multiple/DRLQC/params_lambda/iotmp/".format(args.dist, args.noise_dist)
        rawpath = "./results/{}_{}/finite/multiple/DRLQC/params_lambda/iotmp/raw/".format(args.dist, args.noise_dist)
    else:
        path = "./results/{}_{}/finite/multiple/DRLQC/params_thetas/iotmp/".format(args.dist, args.noise_dist)
        rawpath = "./results/{}_{}/finite/multiple/DRLQC/params_thetas/iotmp/raw/".format(args.dist, args.noise_dist)
    ...
    if args.dist=='normal':
        ...
        theta_v_list = [0.1, 1.0, 2.0, 3.0, 4.0, 6.0]
        theta_w_list = [0.1, 1.0, 2.0, 3.0, 4.0]
    ...
    #REMOVE BELOW
    # drlqc_optimal_theta_w = 1.0
    # drlqc_optimal_theta_v = 3.0
    # drce_optimal_theta_w = 2.0
    # drce_optimal_theta_v = 2.0
    # wdrc_optimal_theta_w = 5.0
