...
def main(dist, noise_dist1, num_sim, num_samples, num_noise_samples, T):
    ...
    if dist=='normal':
        num_samples=num_noise_samples=20
        num_x0_samples=20
    else:
    ...
    theta_w_list = [0.1, 0.2, 0.5, 1.0] # radius of disturbance ambiguity set
    theta_v_list = [0.1, 0.2, 0.5, 1.0] # radius of noise ambiguity set
    ...
    WDRC_DRKF_lambda = np.zeros((len(theta_w_list),len(theta_v_list)))
    WDRC_DRMMSE_lambda = np.zeros((len(theta_w_list),len(theta_v_list)))
