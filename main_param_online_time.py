...
# This file generates data for Online Computation time
...
def main(dist, noise_dist1, num_sim, num_samples, num_noise_samples, T):
    ...
    #----------------------------
    # You can change theta_v list and lambda_list ! but you also need to change lists at plot_params4_drlqc_zeromean.py to get proper plot
    
    theta_v_list = [5.0, 10.0, 15.0] # radius of noise ambiguity set
    if dist=='normal':
        theta_v_list = [1.0] # radius of noise ambiguity set
        theta_w_list = [1.0] # radius of noise ambiguity set
    else:
        theta_v_list = [1.0] # radius of noise ambiguity set
        theta_w_list = [1.0] # radius of noise ambiguity set
        ...
        for idx_w, dist_parameter in enumerate(dist_parameter_list):
            for idx_v,theta in enumerate(theta_v_list):
                for num_noise in num_noise_list:
                    ...
                    print('---------------------')
                    LQG_online_time, WDRC_online_time, DRLQC_online_time, DRCE_online_time = [], [], [], []
                    #----------------------------
                    ...
                    for i in range(num_sim):
                        ...
                        DRCE_online_time.append(output_drce['comp_time'])
                        if i%50==0:
                    ...
                    for i in range(num_sim):
                        ...
                        DRLQC_online_time.append(output_drlqc['comp_time'])
                        if i%50==0:
                    ...
                    for i in range(num_sim):
                        ...
                        WDRC_online_time.append(output_wdrc['comp_time'])
                        if i%50==0:
                    ...
                    for i in range(num_sim):
                        ...
                        LQG_online_time.append(output_lqg['comp_time'])
                        if i%50==0:
                    ...
                    print("LQG : " , np.mean(LQG_online_time, axis=0), "(", np.std(LQG_online_time, axis=0),")")
                    print("WDRC : " , np.mean(WDRC_online_time, axis=0), "(", np.std(WDRC_online_time, axis=0),")")
                    print("DRLQC : " , np.mean(DRLQC_online_time, axis=0), "(", np.std(DRLQC_online_time, axis=0),")")
                    print("DRCE : " , np.mean(DRCE_online_time, axis=0), "(", np.std(DRCE_online_time, axis=0),")")
                    
    # print("Params data generation Completed !")
    # print("If you made any changes, Please make sure your lambda_list(or theta_w_list) and theta_v_list in plot file is as desired")
    
    # if use_lambda:
    #     print("Now use : python plot_params4_drlqc_zeromean.py --use_lambda --dist "+ dist + " --noise_dist " + noise_dist)
    # else:
    #     print("Now use : python plot_params4_drlqc_zeromean.py --dist "+ dist + " --noise_dist " + noise_dist)
