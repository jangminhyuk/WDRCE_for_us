...
def main(dist, noise_dist, num_sim, num_samples, num_noise_samples, T):
    ...
    if dist=='normal':
        ...
        theta_v_list = [0.00001] # radius of noise ambiguity set
        theta_w_list = [0.00001] # radius of noise ambiguity set
    else:
        theta_v_list = [0.01] # radius of noise ambiguity set
    ...
    theta_x0 = 0.001 # radius of initial state ambiguity set
    ...
    if dist == "normal":
        ...
        #disturbance distribution parameters
        w_max = 0.02*np.ones(nx)
        w_min = -0.02*np.ones(nx)
    ...
    if noise_dist =="normal":
        ...
        v_min = -0.05*np.ones(ny)
        v_max = 0.05*np.ones(ny)
    ...
    def perform_simulation(lambda_, noise_dist, dist_parameter, theta, idx_w, idx_v):
        for num_noise in num_noise_list:
            ...
            if use_lambda:
                path = "./results/{}_{}/finite/multiple/DRLQC/params_lambda/air/".format(dist, noise_dist)
            else:
                path = "./results/{}_{}/finite/multiple/DRLQC/params_thetas/air/".format(dist, noise_dist)
            ...
            #Save all raw data
            if use_lambda:
                rawpath = "./results/{}_{}/finite/multiple/DRLQC/params_lambda/air/raw/".format(dist, noise_dist)
            else:
                rawpath = "./results/{}_{}/finite/multiple/DRLQC/params_thetas/air/raw/".format(dist, noise_dist)
